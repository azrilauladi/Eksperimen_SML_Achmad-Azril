...
Script untuk melakukan preprocessing data Wine Quality secara otomatis.
...
def drop_unnecessary_columns(df: pd.DataFrame, columns: list = ['Id']) -> pd.DataFrame:
    ...
    Args:
        df: DataFrame input
        columns: List kolom yang akan dihapus
        
    Returns:
        DataFrame tanpa kolom yang dihapus
    """
...
def cap_outliers_iqr(df: pd.DataFrame, columns: list) -> pd.DataFrame:
    """
    Menangani outliers menggunakan IQR capping method.
    
    Args:
        df: DataFrame input
        columns: List kolom untuk di-cap
        
    Returns:
        DataFrame dengan outliers yang sudah di-cap
    """
...
def split_features_target(df: pd.DataFrame, target_column: str = 'quality'):
    """
...
def save_preprocessed_data(X_train, X_test, y_train, y_test, scaler, output_dir: str):
    ...
    # Simpan ke CSV
    train_path = os.path.join(output_dir, 'WineQT_train.csv')
    test_path = os.path.join(output_dir, 'WineQT_test.csv')
    all_path = os.path.join(output_dir, 'WineQT_preprocessing.csv')
...
def preprocess(input_path: str, output_dir: str, test_size: float = 0.2, random_state: int = 42):
    """
    Fungsi utama untuk melakukan preprocessing secara lengkap.
    ...
    Returns:
        Dictionary berisi data yang sudah dipreprocessing
    """
    print("=" * 60)
    print("WINE QUALITY DATA PREPROCESSING")
    print("=" * 60)
    ...
    # 5. Split features dan target
    X, y = split_features_target(df, target_column='quality')
...
def get_preprocessed_data(output_dir: str):
    ...
    Args:
        output_dir: Direktori tempat file disimpan
        
    Returns:
        Dictionary berisi data train, test, dan scaler
    """
    train_path = os.path.join(output_dir, 'WineQT_train.csv')
    test_path = os.path.join(output_dir, 'WineQT_test.csv')
    scaler_path = os.path.join(output_dir, 'scaler.pkl')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Wine Quality Data Preprocessing')
    parser.add_argument('--input', type=str, default='../WineQT_raw/WineQT.csv',
